...
class ReporteView:
    ...
    def exportar_excel(self):
        try:
            ...
            reportes = []
            for item in self.tabla_reportes.get_children():
                reportes.append(self.tabla_reportes.item(item)['values'])
            
            success, message = exportador.exportar_excel("reportes", reportes)
            
            if success:
                self.mostrar_mensaje("Éxito", message)
            else:
                if "cancelada" not in message.lower():
                    self.mostrar_mensaje("Error", message, "error")
    ...
    def exportar_pdf(self):
        try:
            ...
            success, message = exportador.exportar_pdf("reportes", reportes)
