...
class UsuarioController:
    ...
    def guardar_usuario(self, username, email, password, imagen=None):
        print(f"DEBUG: guardar_usuario llamado con - username: {username}, email: {email}")
        
        success, message = self.model.crear_usuario(username, email, password)
        
        print(f"DEBUG: Resultado del modelo - success: {success}, message: {message}")
